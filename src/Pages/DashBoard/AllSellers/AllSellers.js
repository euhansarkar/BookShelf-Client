...
const AllUsers = () => {
  const {
    ...
    queryKey: [`users`],
    queryFn: async () => {
      const res = await fetch(`https://products-resale-server-euhansarkar.vercel.app/sellers`);
  ...
  const handleIsApproved = (id) => {
    fetch(`https://products-resale-server-euhansarkar.vercel.app/users/seller/${id}`, {
      method: `PUT`,
      headers: {
