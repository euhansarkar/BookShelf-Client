...
const ShowCategoryThree = () => {
  const [chooseProduct, setChooseProduct] = useState(null);
  const [booksData, setBooksData] = useState([]);
  useEffect(() => {
    fetch(`https://products-resale-server.vercel.app/products/develop`)
      .then((res) => res.json())
  ...
  return (
    <>
      <div className="flex items-center justify-between flex-wrap mx-8">
        <span className="font-bold text-3xl capitalize">personal development books</span>
      ...
      >
        {booksData.map((book) => (
          <SwiperSlide className="mb-20 mt-6 px-8 md:px-0 md:mx-8" key={book._id}>
            <SingleProduct book={book} chooseProduct={chooseProduct}
...
export default ShowCategoryThree;
